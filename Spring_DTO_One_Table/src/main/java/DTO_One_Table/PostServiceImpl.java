...
public class PostServiceImpl implements PostService {
	...
	public PostResponse mySave(PostDto postdto) {
		...
		System.out.print("show Dto " + postdto.getFirstName() + postdto.getLastName() + postdto.getDob()
				+ postdto.getEmail() + postdto.getMobile());
	...
	public List<PostDto> showAll() {
		...
		for (Post emp : post) {
			list.add(new PostDto(emp.getId(), emp.getFirstName(), emp.getLastName(), emp.getDob(), emp.getEmail(),
					emp.getMobile()));
		}
		return list;
	...
	public PostDto showById(Long id) {
		...
		if (optional.isPresent()) {
			return (new PostDto(post.getFirstName(), post.getLastName(), post.getDob(), post.getEmail(),
					post.getMobile()));
		} else {
...
//		Post post = new Post(); 
	...
	public List<PostDto> showAll_userQuery_1() {
		List<PostDto> list = new ArrayList<>();
		List<Post> post = this.postRepository.getAll();
		for (Post emp : post) {
			list.add(new PostDto(emp.getId(), emp.getFirstName(), emp.getLastName(), emp.getDob(), emp.getEmail(),
					emp.getMobile()));
		}
	...
	@Override
	public PostDto UpadateUserQuery(String name, Long id) {
		Integer test = this.postRepository.updateUserNameByID(name, id);
		PostDto postdto;
		if(test == 1) {
		Optional<Post> post = postRepository.findById(id);
		Post po = post.get();
		postdto = new PostDto(po.getId(), po.getFirstName(), po.getLastName(), po.getDob(), po.getEmail(),
					po.getMobile());
		return postdto;
		}
		else {
	...
	public List<String> nameAll() {
		List<String> str = postRepository.getNameAll();
		return str;
	}
